...
##############################  Spatial_QLeNet_MCME   ####################################
def S_Qlenet_mcme(args, num_layers, name="s_qlenet"):
    ...
    orig_x = BayesianDropout(args.dropout_rate)(x)
    ...
    for i in range(mc_samples - 1):
      mc_x = BayesianDropout(args.dropout_rate)(ic_x[i])
    ...
    mc_model = Model(input, [orig_x] + mc_xs, name=name+"_mc")
...
def S_QResNet18_mcme(classes, input_shape, args, weight_decay=1e-4, base_filters=64, name="s_resnet18"):
    mc_samples = args.mc_samples
    input = Input(shape=input_shape)
    x = input
...
def S_QVGG11_mcme(args, num_bayes_loc=7, filters=64, dense_out=[512, 512, 100], name="s_qvgg"):
    input_shape=(32,32,3)
    input = Input(shape=input_shape)
    x = input
    mc_samples = args.mc_samples
    ...
    orig_x = BayesianDropout(args.dropout_rate)(orig_x)
