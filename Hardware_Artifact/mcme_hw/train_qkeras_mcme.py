...
from t_qmodels_mcme import T_Qlenet_mcme, T_QVGG11_mcme, T_QResNet18_mcme
from s_qmodels_mcme import S_Qlenet_mcme, S_QVGG11_mcme, S_QResNet18_mcme
...
def get_model(args):
    ...
    if args.model_name == "lenet":
        if args.opt_mode == "temporal":
            model, mc_model = T_Qlenet_mcme(args, model_num_layer[args.model_name])
        else:
            model, mc_model = S_Qlenet_mcme(args, model_num_layer[args.model_name])
    elif args.model_name == "resnet":
        if args.opt_mode == "temporal":
            model, mc_model = T_QResNet18_mcme(input_shape=(32, 32, 3), classes=10, args=args, weight_decay=1e-4, base_filters=16)
        else:
            model, mc_model = S_QResNet18_mcme(input_shape=(32, 32, 3), classes=10, args=args, weight_decay=1e-4, base_filters=16)
    elif args.model_name == "vgg":
        if args.opt_mode == "temporal":
            model, mc_model = T_QVGG11_mcme(args, filters=16, dense_out=[16, 16, 10])
        else:
            model, mc_model = S_QVGG11_mcme(args, filters=16, dense_out=[16, 16, 10])
    else:
...
if __name__ == '__main__':
    ...
    if (args.opt_mode == "spatial") and args.is_train:
        weight = model.get_layer('fc_final').get_weights()
        for i in range(args.mc_samples-1):
            layer_name = 'mc_fc_final_%i'%(i)
            mc_model.get_layer(layer_name).set_weights(weight)

    if args.save_model is not None: mc_model.save(args.save_model+'.h5')
