...
class FullAnalysis():
    ...
    def saver(self, experiment_id):
        ...
        with open("test_evaluation_log_"+str(experiment_id)+".txt", "w") as file:
            ...
            for layer in range(len(self.cur_correct_saver)):
                ...
                if self.exit_only:
                    print(f"E ({dropout_rate},{str(layer)}), {str(self.accu_saver[layer])}, {str(self.ece_saver[layer])}, {str(self.get_flops_standard_exit(layer, mc_passes, ensemble = False)/self.baseline_flops)}")
                    print(f"E ({dropout_rate},Ensemble{str(layer)}), {str(self.ensemble_accu_saver[layer])}, {str(self.ensemble_ece_saver[layer])}, {str(self.get_flops_standard_exit(layer, mc_passes, ensemble = True)/self.baseline_flops)}")
    ...
    def get_confidence_exiting_values(self, model_num):
        ...
        for threshold in confidence_list:
            ...
            if self.exit_only:
                print(f"E ({dropout_rate},{threshold}), {accuracy}, {ece}, {flops/(self.baseline_flops*10000)}")
                print(f"Ensemble E ({dropout_rate},{threshold}), {ensembled_accuracy}, {ensembled_ece}, {ensembled_flops/(self.baseline_flops*10000)}")
            elif self.model.dropout == "block":
